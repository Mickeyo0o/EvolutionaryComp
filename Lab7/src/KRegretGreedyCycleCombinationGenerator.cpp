...
std::vector<int> KRegretGreedyCycleCombinationGenerator::generateCycle(int start_pos, std::vector<int> startingCycle)
{
    int n = costDistanceInfo->getNumOfNodes();
    std::vector<int> cycle = startingCycle;
    bool* visited = new bool[n];
    for(int i = 0; i < n; i++) {
        visited[i] = false;
    }
    for(int id: startingCycle)
    {
        visited[id] = true;
    }

    for(int elInCycle = cycle.size(); elInCycle < nodesInCycle; elInCycle++) {
        int* bestPosition = new int[n];
        float* objValues = new float[n];
        for(int i = 0; i < n; i++) {
            bestPosition[i] = -1;
